...
std::pair<Entry, Entry> TableManager::undoPolicy(const ID_t& i_policy_id) {
  INF("enter TableManager::undoPolicy().");
  std::shared_ptr<DateTime> ptr_datetime;
  Policy policy = this->m_policy_table.readPolicy(i_policy_id, ptr_datetime);
  Entry source_entry = this->undo(policy.getSourceID());
  DBG3("Undone policy [ID: %lli], rolled back source entry [ID: %lli].",
       i_policy_id, policy.getSourceID());
  Entry destination_entry = this->undo(policy.getDestinationID());
  DBG3("Undone policy [ID: %lli], rolled back source entry [ID: %lli].",
       i_policy_id, policy.getDestinationID());
  INF("exit TableManager::undoPolicy().");
  return (std::make_pair(source_entry, destination_entry));
}
